...
function postFetch(image_url, title, genre, author, stanza, category_id){
  const bodyData = {image_url, title, genre, author, stanza, category_id} /// only with the scope '"BodyData can't change"
  fetch(myAPI, {
    method: "POST",
    headers: {"Content-Type": "application/json"},
    body: JSON.stringify(bodyData)
  })
  .then(response => response.json()) //  reutrns the data if the promise if forfilled
  .then(poem => { 
...
function updateFormHandler(e){ // this "e" represents the event listener  it allows if you pass it in if you need it  or don't pass it in if you don't need it 
  e.preventDefault(); // I am preventing the deafualt beahavior button from refreahing the page
...
function patchPoem(title, genre, author, stanza, image_url, category_id, poem){
  ...
  .then(res => res.json())
  .then(updatePoem => { 
    let poem = Poem.findById(updatePoem.data.id); // we are giving an id so we can find that the poem instance
